...
receive("FProfile_getSource", function(_, ply)
	...
	local info = func and debug.getinfo(func) or {}
	info.short_src = info.short_src or net.ReadString()
	info.linedefined = info.linedefined or net.ReadUInt(16)
	info.lastlinedefined = info.lastlinedefined or net.ReadUInt(16)
	local source = FProfiler.readSource(info.short_src, info.linedefined, info.lastlinedefined) or ""
...
receive("FProfile_printFunction", function(_, ply)
	...
	for _, row in ipairs(dataSource or {}) do
		if tostring(row.func) == func then
			data = row
			break
		end
	end
...
receive("FProfile_fullModelUpdate", function(_, ply)
	...
	if model.focusObj ~= nil then
		net.WriteString(tostring(model.focusObj))
	end

	-- Bool also indicates whether it's currently profiling
	net.WriteBool(model.sessionStart ~= nil)

	if model.sessionStart ~= nil then
		net.WriteDouble(model.sessionStart)
	end
